...
// It also is setup to use 24 bit output, so it may only work on T4.1 with
// psram or some of the "Dev" boards which have SDRam...
...
uint32_t *frame_buffer_RGB888 = nullptr;
bool g_use_efb = true;
...
bool g_last_picture_rgb888 = false;
...
inline void FillScreenOrFB(uint16_t color) {
    if (frame_buffer_RGB888) {
        uint8_t r, g, b;
        tft.color565toRGB(color, r, g, b);
        if (r) r |= 0x7;
...
#define BUFFPIXEL 80
void bmpDraw24(File &bmpFile, const char *filename, bool fErase) {
            ...
            if ((bmpDepth == 24) && (read32(bmpFile) == 0)) {  // 0 = uncompressed
                ...
                if (g_image_scale_up) {
                    ...
                    if (fErase && (((image_width * g_image_scale_up) < g_tft_width) || ((image_height * g_image_scale_up) < image_height))) {
                        FillScreenOrFB((uint16_t)g_background_color);
                    }
...
void ScaleUpwriteClippedRect24(int row, int image_width, uint32_t *usPixels) {
    ...
    for (int col = 0; col < image_width; col++) {
        ...
        *p = *ppixIn++;  // copy the pixel in to pixel out
        if (col) {
            ...
            tft.color888toRGB(*p, r_cur, g_cur, b_cur);
            tft.color888toRGB(*(p - g_image_scale_up), r_prev, g_prev, b_prev);
            for (int j = 1; j < g_image_scale_up; j++) {
                ...
                *(p - g_image_scale_up + j) = tft.color888(red, green, blue);
            }
        }
        p += g_image_scale_up;
    }
    ...
    if (row) {
        for (int col = 0; col < image_width; col++) {
            tft.color888toRGB(*puCurRow++, r_cur, g_cur, b_cur);
            tft.color888toRGB(*puPrevRow++, r_prev, g_prev, b_prev);
        ...
        for (uint32_t pix_cnt = 0; pix_cnt < (uint32_t)image_width; pix_cnt += g_image_scale) {
            ...
            for (uint8_t i = 0; i < g_image_scale; i++) {
                for (uint8_t j = 0; j < g_image_scale; j++) {
                    tft.color888toRGB(usPixels[pix_cnt + i + (j * image_width)], red, green, blue);
            ...
            usPixels[newx++] = tft.color888((uint8_t)sqrt(r / (g_image_scale * g_image_scale)), (uint8_t)sqrt(g / (g_image_scale * g_image_scale)), (uint8_t)sqrt(b / (g_image_scale * g_image_scale)));
        }
...
void ScaleDownWriteClippedRect(int row, int image_width, uint16_t *usPixels) {
    if ((row % g_image_scale) == (g_image_scale - 1)) {
        ...
        tft.writeRect(0, row / g_image_scale, image_width / g_image_scale, 1, usPixels);
    }
}
